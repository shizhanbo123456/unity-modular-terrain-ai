...
namespace UnityPythonBridge
{
    ...
    public static class BridgeDispatcher
    {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                foreach (var type in types)
                {
                    foreach (var method in type.GetMethods(BindingFlags.Public | BindingFlags.Static))
                    {
                        ...
                        // 校验签名：object Name(BridgeContext, JObject)
                        var pars = method.GetParameters();
                        if (pars.Length != 2 ||
                            pars[0].ParameterType != typeof(BridgeContext) ||
                            pars[1].ParameterType != typeof(JObject))
                        {
                            UnityEngine.Debug.LogWarning(
                                $"[UnityPythonBridge] 忽略命令方法 {type.Name}.{method.Name}：签名必须为 (BridgeContext, JObject)");
                            continue;
                        }
        ...
        public static object Execute(string command, JObject args)
        {
            ...
            return info.Handler(new BridgeContext(), args ?? new JObject());
        }
